...
EipStatus HandleNullNonMatchingForwardOpenRequest(
  ...
  CipMessageRouterResponse *message_router_response
	...
	*/
	...
	EipUint32 temp = ParseConnectionPath_NFO(&g_dummy_connection_object,
	...
	if (g_config_data_length > 0)
	{
		 ...
		 *
		 * •If the entire configuration cannot be applied then none of the configuration shall be
		 * applied and the appropriate error code returned.
		 *
		...
		OPENER_TRACE_INFO("No configuration data found!\n"); //TODO: remove ??
...
EipUint8 CheckForwardOpenHeaderLength(CipConnectionObject *connection_object,
	...
	/*packet-length as expected */
	return kEipStatusOk;

}

EipStatus CheckElectronicKey(CipConnectionObject *connection_object,  //TODO: move to cipelectronickey.c
		CipMessageRouterRequest *message_router_request, size_t remaining_path,
		EipUint16 *extended_status) {
...
//             sizeof(connection_object->produced_path) );
      ...
      connection_object->produced_connection_path = NULL;

      /* configuration data should be found in the path */
